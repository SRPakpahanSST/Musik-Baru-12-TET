...
async function loadPartiturHTML() {
    try {
        ...
        if (container) {
            ...
            // Panggil init setelah HTML dimuat
            initPartiturEvents();
        } else {
        ...
        const container = document.getElementById('partitur-container');
        if (container) {
            container.innerHTML = '<p style="color:red; text-align:center;">⚠️ Error: ' + error.message + '</p>';
        }
    }
}
...
function cetakPartitur() {
    const partiturText = document.getElementById('partitur-area').value;
